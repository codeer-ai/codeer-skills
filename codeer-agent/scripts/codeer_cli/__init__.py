...
- Cowork: pass them as CLI flags, add them to ``session.env``, or export them.

Auth (CODEER_API_BASE, CODEER_SESSION_ID, CODEER_CSRF_TOKEN) is resolved from
``~/.codeer/session.env`` or a repo-root ``session.env``. See
``API_CHEATSHEET.md`` -> "Session config" for the rationale.
"""
