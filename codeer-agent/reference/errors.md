...
| Request timed out while creating or sending a history turn | The server did not respond before the per-message timeout | Run `codeer history conversations <history_id> --out .codeer/current/history-<history_id>.json` before retrying. The write outcome is uncertain and retrying blindly can duplicate a turn. |
